      ...
      <div className="relative h-[420px] overflow-hidden">
        ...
        <div className="absolute bottom-7 left-8 right-8 flex items-end justify-between z-10">
          ...
          <button
            ...
            className={[
              "flex items-center gap-2 rounded-full px-5 py-3 text-sm font-medium border transition-all flex-shrink-0 self-end",
              saved
          ...
          <Section title="Tags">
            <div className="flex flex-wrap gap-2">
              {activity.tags.map(tag => (
                <span
                  key={tag}
                  className="px-4 py-2 rounded-full border border-[#EAE8E2] text-sm font-medium text-[#1C1C1A] bg-white"
                >
                  {tag}
                </span>
              ))}
        ...
        <div>
          ...
          <div className="bg-white border border-[#EAE8E2] rounded-[22px] overflow-hidden mb-5">
            {[
              ...
              <div
                key={label}
                className={`flex items-center gap-3.5 px-5 py-4 ${i < arr.length - 1 ? "border-b border-[#EAE8E2]" : ""}`}
              >
                <div className="w-9 h-9 rounded-full bg-[#D4EAC8] flex items-center justify-center text-base flex-shrink-0">
                  {icon}
                </div>
                <div>
          ...
          <div className="relative h-40 rounded-[22px] overflow-hidden border border-[#EAE8E2] bg-[#EEF5E8] mb-3">
            <div className="absolute inset-0"
              style={{
                background: "repeating-linear-gradient(0deg, transparent, transparent 29px, rgba(107,160,80,.1) 30px), repeating-linear-gradient(90deg, transparent, transparent 29px, rgba(107,160,80,.1) 30px)"
              }}
            />
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-5 h-5 bg-[#1E3A1E] shadow-md z-10"
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-10">
      <div className="text-[11px] font-bold tracking-[.1em] uppercase text-[#9A9590] mb-4">
        {title}
      </div>
      {children}
    </div>
